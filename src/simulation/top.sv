...
class generator extends uvm_sequence;
	...
	virtual task body();
		for (int i = 0; i < num; i++) begin
            ...
            start_item(item);
            // "one-hot" izbor operacije: 0..5 = šest komandi, 6 = hold
            ...
            item.sr  = (op == 4);
            item.sl  = (op == 5);
            item.ir  = $urandom_range(0, 1);
            item.il  = $urandom_range(0, 1);
            item.in  = $urandom_range(0, 15);
...
class driver extends uvm_driver #(reg4_item);
    ...
    virtual function void build_phase(uvm_phase phase);
        ...
        if (!uvm_config_db#(virtual reg4_if)::get(this, "", "reg4_vif", vif)) `uvm_fatal("Driver", "No interface.");
    endfunction
...
// Interface
interface reg4_if(input bit clk);
